...
function ConnectBuilder(urlPath) {
  ...
  */
  // app stores all the middleware added
  this.app = connect();
...
ConnectBuilder.prototype.authorize = function authorize(user, pass) {
  // *use* is used to add a middleware
  this.app.use(
    this.urlPath,
    basicAuth(
...
ConnectBuilder.prototype.index = function index(
  ...
  // probably: middlware that serves an index of the directory in the given path.
  this.app.use(this.urlPath, (req, res) => {
    fs.readFile(path, (err, data) => {
      ...
      res.end(
        data
          .toString('utf-8')
          // these are placeholders in `web/index.html`
